(() => {
  ...
  function buildTraversal(position = "top") {
    ...
    if (position === "bottom") {
      ...
      prompt.textContent = "Finished this chapter? Pick the next one right here.";
  ...
  function wireStickyControls() {
    ...
    const cluster = document.getElementById("stickyCluster");
    const topBtn = document.getElementById("scrollToSearchBtn");
    const bottomBtn = document.getElementById("scrollToBottomTraversalBtn");
    if (!cluster || !topBtn || !bottomBtn) return;

    topBtn.addEventListener("click", () => {
      const target = document.getElementById("worksNav") || document.querySelector(".hero");
      if (target) {
    ...
    const syncVisibility = () => {
      cluster.classList.toggle("visible", window.scrollY > Math.max(240, window.innerHeight * 0.45));
    };
  ...
  function clearRefreshTimers() {
    if (railRefreshTimer) {
      clearInterval(railRefreshTimer);
